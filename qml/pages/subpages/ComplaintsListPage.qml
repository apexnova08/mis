...
Rectangle
{
    ...
    QtObject
    {
        ...
        {
            let searchString = "SELECT * FROM complaints WHERE "
            ...
            if (txtSearchName.text != "")
            {
                ...
                {
                    searchString = searchString + "(comComplainant LIKE '%" + names[i] + "%' OR "
                    searchString = searchString + "comDefendant LIKE '%" + names[i] + "%' OR "
                    searchString = searchString + "comTitle LIKE '%" + names[i] + "%') AND "
                }
            }
            if (comboStatus.currentIndex != 0)
            {
                searchString = searchString + "comStatus = '" + comboStatus.currentValue + "' AND "
            }
            if (comboFMonth.currentIndex != 0)
            {
                ...
                searchString = searchString + "comDate LIKE '%-" + str + "-%' AND "
            }
            ...
            if (txtFYear.text != "")
            {
                searchString = searchString + "comDate LIKE '" + txtFYear.text + "-%' AND "
            }

            // Finalization
            if (searchString == "SELECT * FROM complaints WHERE ")
            {
                searchString = "SELECT * FROM complaints"
            }
            else
            {
                searchString = searchString.slice(0, -5)
            }
            backend.SearchComplaint(searchString)
        }
    }
    ...
    TextField
    {
        id: txtSearchName
        selectByMouse: true
        placeholderText: "Search complaint by name or title..."
    ...
    Rectangle
    {
        ...
        ScrollView
        {
            ...
            {
                id: comboStatus
                model: ["Any", "Unsettled", "Settled", "Filed for action"]
    ...
    Button
    {
        ...
        onClicked: backend.OpenAddComplaintPanel()
    }
}